...
import { stockAtom, loggedinAtom } from '../utils/atoms'
...
export default function Home({ products }: ShopifyProductProps) {
  ...
  async function filterProducts() {
    ...
    const res = await fetch(`${BACKEND_URL}api/products/shopify?vendor=${searchTerms}`)
    const data = await res.json()
    setFilteredProducts(data)
  }

  async function fetchItems() {
    const res = await fetch(`${BACKEND_URL}api/products/ebay?q=${queryTerm}`)
    const data = await res.json()
  ...
  async function changeInventory() {
    ...
    const res = await fetch(`${BACKEND_URL}api/products/update`, requestOptions)
    if (res.status) {
      setShowModal(false)
    }
  }
